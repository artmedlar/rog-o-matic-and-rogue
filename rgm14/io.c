 ...
 */
# define charonscreen(X,Y)	((char)(mvinch(X,Y) & A_CHARTEXT))
...
{ int   botprinted = 0, wasmapped = didreadmap, r, c, pending ();
  ...
  char *cont = "continue--";		/* FSM for '--press space to continue--' */
  char *rmore = "-more-";		/* FSM for rogue-clone's '-more-' */
  ...
  DLOG("    getrogue: row=%d col=%d scr='%c'(%d) onat=%d\n", row, col, screen[row][col], (int)screen[row][col], onat);
  /* onat=3 means we sent a command and must read at least one token before checking '@' */
  { int need_first = (onat == 3);
  while (need_first || (*s) ||
         ((!hasted || version != RV36A) && onat && screen[row][col] != '@'))
  { ch = getroguetoken ();
    need_first = 0;  /* Got at least one token */
    DLOG("      tok=%d row=%d col=%d scr=%d\n", (int)ch, row, col, (int)screen[row][col]);
    ...
    /* Handle rogue-clone's '--press space to continue--' */
    if (ch == *cont)
    { if (*++cont == 0)
      { DLOG("      CONT detected, sending space + ctrl-L\n");
        sendnow (" ");   /* Just space for rogue-clone */
        sendnow ("%c", 12);   /* Ctrl-L to redraw screen */
        cont = "continue--";
      }
    }
    else cont = "continue--";

    /* Handle rogue-clone's '-more-' prompt */
    if (ch == *rmore)
    { if (*++rmore == 0)
      { DLOG("      -more- detected, sending space\n");
        sendnow (" ");   /* Space to continue */
        rmore = "-more-";
      }
    }
    else rmore = "-more-";
	...
	standout ();
      ...
      case TIMEOUT_TOK:
        /* Timeout waiting for input - send space to dismiss any prompt */
        DLOG("      TIMEOUT - sending space and exiting getrogue loop\n");
        sendnow(" ");  /* Dismiss inventory, --more--, or other prompts */
        goto done;
        break;
...
void sendnow(const char *f, ...)
{ char cmd[128];
  va_list ap;
  va_start(ap, f);
  vsnprintf(cmd, sizeof(cmd), f, ap);
  va_end(ap);
  for (char *s = cmd; *s; s++) sendcnow(*s);
  fflush(trogue);
}
...
{ register char *l=lastcmd;		/* Ptr into last command */
  ...
  *l = '\0';
  DLOG("    resend: sent '%s'\n", lastcmd);
  fflush (trogue);			/* Flush output to rogue */
...
/* Helper: get char from rogue with timeout, returns TIMEOUT_TOK on timeout */
static char getroguechar_timeout()
{ 
  fd_set fds;
  struct timeval tv;
  int fd = fileno(frogue);
  unsigned char buf[1];
  ssize_t n;
  ...
  /* Use raw read() to bypass stdio buffering issues */
  n = read(fd, buf, 1);
  if (n <= 0) {
    return (n == 0) ? EOF : TIMEOUT_TOK;
  }
...
{ char ch;
  ...
  ch = getroguechar_timeout();
  if (ch == TIMEOUT_TOK) return TIMEOUT_TOK;
  ...
  if ((int) ch == CM_TOK)
  { char r = getroguechar_timeout();
    if (r == TIMEOUT_TOK) return TIMEOUT_TOK;
    char c = getroguechar_timeout();
    if (c == TIMEOUT_TOK) return TIMEOUT_TOK;
    row = (int) r - 32; col = (int) c - 32;
  }
...
{ register char *vstr = versionstr, *m = VERMSG;
  ...
  { int i;
    ...
    sendnow ("v");
    getrogue ("", 1);			/* Read version message, wait for cursor on @ */
    
    /* Extract version from screen[0] by finding "ersion " */
    for (i = 0; i < 70; i++)
      if (screen[0][i] == 'e' && screen[0][i+1] == 'r' && screen[0][i+2] == 's' &&
          screen[0][i+3] == 'i' && screen[0][i+4] == 'o' && screen[0][i+5] == 'n' &&
          screen[0][i+6] == ' ')
      { p = &screen[0][i+7];
        while (*p != ' ' && *p != '.' && vstr < versionstr + 30) *(vstr++) = *p++;
        *vstr = '\0';
        break;
      }
    if (i >= 70) sprintf (versionstr, "Clone");  /* Default if not found */
  }
  ...
  else if (stlmatch (versionstr, "Clone"))	version = RV53A;  /* rogue-clone uses 5.3 style */

...
/* Modern ncurses provides unctrl() as a function - no fallback needed */

/* Debug tracing macro - use -D flag to enable */
#define DLOG(fmt, ...) do { if (debug_trace && debug_file) { fprintf(debug_file, fmt, ##__VA_ARGS__); fflush(debug_file); } } while(0)

/* Global Preprocessor constants */

/* Original sync pattern for BSD rogue - doesn't work with rogue-clone */
/* # define ill		";'" */
/* Empty string - rely on cursor-on-@ detection instead */
# define ill		""
# define status		"Str:"
...
# define ISPRT(c)	((c) >= ' ' && (c) <= '~')
/* Helper function in io.c for timeout-based reading */
int getroguechar_with_timeout(void);

...
{ int score, best = -1;
  ...
  char *s;
  char tmpfname[] = TEMPFL;  /* Must be modifiable for mkstemp */
  FILE *tmpfil;

  /* Skip score checking - rogue -s requires a terminal which we don't have */
  /* TODO: Re-enable when rogue works with pipes */
  return best;

  /* Run 'rogue -s', and put the scores into a temp file */
  int fd = mkstemp(tmpfname);
  if (fd >= 0) close(fd);
  sprintf (cmd, "%s -s >%s 2>/dev/null", rogue, tmpfname); 
  system (cmd); 

...
/* Debug tracing */
int   debug_trace = 0;		/* True if debug tracing enabled */
FILE *debug_file = NULL;	/* Debug output file */
int   moverate = 0;		/* Max moves per second (0 = unlimited) */
...
char roguename[128] = "Rog-O-Matic";
...
{ char  ch, *s, *getenv(), *statusline(), msg[128];
  ...
  { int rfd = argv[1][0] - 'a';
    int wfd = argv[1][1] - 'a';
    // // fprintf(stderr, "DEBUG: read fd=%d, write fd=%d\n", rfd, wfd);
    frogue = fdopen (rfd, "r");
    trogue = fdopen (wfd, "w");
    if (!frogue) { fprintf(stderr, "fdopen frogue (fd=%d) failed\n", rfd); exit(1); }
    if (!trogue) { fprintf(stderr, "fdopen trogue (fd=%d) failed\n", wfd); exit(1); }
    setvbuf (frogue, NULL, _IONBF, 0);  /* Unbuffered input from rogue */
    setvbuf (trogue, NULL, _IONBF, 0);  /* Unbuffered output to rogue */
    // // fprintf(stderr, "DEBUG: streams setup OK\n"); fflush(stderr);
  }

  // // fprintf(stderr, "DEBUG: parsing args, argc=%d\n", argc); fflush(stderr);
  ...
  if (argc > 3) {
    sscanf (argv[3], "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
			...
			&emacs, &terse, &transparent, &quitat, &debug_trace, &moverate);
  }
  
  /* Open debug file if tracing enabled */
  if (debug_trace) {
    debug_file = fopen("/tmp/rgm.log", "w");
    if (debug_file) fprintf(debug_file, "Debug tracing enabled, moverate=%d\n", moverate);
  }

  /* The fourth argument is the Rogue name */
  // // fprintf(stderr, "DEBUG: about to set roguename, argc=%d\n", argc); fflush(stderr);
  if (argc > 4) {
    // // fprintf(stderr, "DEBUG: argv[4]=%s\n", argv[4]); fflush(stderr);
    strcpy (roguename, argv[4]);
  }
  else		sprintf (roguename, "Rog-O-Matic %s", RGMVER);
  // // fprintf(stderr, "DEBUG: roguename=%s\n", roguename); fflush(stderr);

  /* Now count argument space and assign a global pointer to it */
  /* DISABLED: modifying argv can crash on modern systems */
  arglen = 0;
  for (i=0; i<argc; i++)
  { register int len = strlen (argv[i]);
    arglen += len + 1;
    /* Don't modify argv - it may be read-only */
    /* while (len >= 0) argv[i][len--] = ' '; */
  }
  parmstr = argv[0];	arglen--;
  // // fprintf(stderr, "DEBUG: arglen=%d\n", arglen); fflush(stderr);

  /* If we are in one-line mode, then squirrel away stdout */
  // // fprintf(stderr, "DEBUG: emacs=%d terse=%d noterm=%d\n", emacs, terse, noterm); fflush(stderr);
  if (emacs || terse)
  ...
  halfdelay(1);				/* getch() returns ERR after 0.1s */
  if (startecho) toggleecho ();		/* Start logging? */
  clear ();				/* Clear the screen */
  getrogue (ill, 2);			/* Read initial screen first */
  ...
  /* For rogue-clone, don't send redraw command - just let getrogue parse 
   * the initial screen that's already been sent. */
  ...
  if (!replaying) {
    /* rogue-clone sends ^L only on initial draw, not after Ctrl-L redraw.
     * Skip waiting for CL_TOK since we already consumed the initial draw
     * during getrogver(). */
  }
  ...
  /* Identify all 26 monsters - DISABLED for rogue-clone which doesn't have / command */
  /* if (!replaying)
  ...
  /* Force clean state before signal setup */
  interrupted = 0;
  transparent = 0;
  ...
  { static int loopcount = 0;
    ...
    { int sent = 0;
      while (resend())
      { DLOG("  resend returned 1, calling getrogue\n");
        if (startingup) showcommand (lastcmd);
        getrogue (ill, 3);  /* onat=3: sent command, must read at least 1 token */
        DLOG("  getrogue done\n");
        sent = 1;
      }
      
      /* Throttle move rate if -m specified (only after we sent something) */
      if (sent && moverate > 0) {
        usleep(1000000 / moverate);
      }
    }
    ...
    { int strat_result = strategize();
      DLOG("  strategize returned %d, transparent=%d\n", strat_result, transparent);
      if ((transparent && !singlestep) ||
	  (!emacs && charsavail ()) ||
          !strat_result)
      { DLOG("  calling getch()\n");
        ch = (noterm) ? ROGQUIT : getch ();
        DLOG("  getch returned %d\n", ch);
      
      /* halfdelay mode: ERR means no key in 0.1s, try strategize again */
      if (ch == ERR) { continue; }
  ...
  /* Reset terminal: clear screen and restore normal line endings */
  printf ("\033[H\033[J");  /* ANSI: home cursor and clear screen */
  system("stty sane 2>/dev/null");  /* Reset terminal to sane state */
  fflush(stdout);
